    ...
    function genera_controlli_js()
    {
      ...
      print "function cancellaFoto(nome,id,cosa)
      		{
      		    ...
      		    {
      			location.href='../include/funzioni/modifica.php?nomeF='+nome+'&id='+id+'&cosa='+cosa;
      		    }
      		}
      ";
      ...
      foreach($this->_properties as $p)
      {
        if(isset($p['onsubmit']))
        {
          ...
          $variabili=explode('&',$_SERVER['QUERY_STRING']);
          $var='';
    ...
    foreach($this->_properties as $p)
      {
			...
			elseif($p['tipo']=='textarea')
        {
          ...
          print "<div class=\"campo_inserimento\"> &nbsp;<textarea id=\"".$p['nome']."\" name=\"".$p['nome']."\" cols=\"35\" rows=\"10\">".$value."</textarea></div>\n";
          print "</div>\n";
        }
        elseif($p['tipo']=='password')
        {
          ...
          print "<div class=\"campo_inserimento\">&nbsp; <input type=\"".$p['tipo']."\" id=\"c".$p['nome']."\" name=\"c".$p['nome']."\" size=\"28\" ".$ch." /></div>\n";
          print "</div>\n";
        }
        ...
        elseif($p['tipo']=='select')
        {
          ...
          print "<div class=\"riga_form\">\n";
          print "<div class=\"label_form\"><label for=\"".$p['nome']."\">".$p['label']." </label></div>\n";
          print "<div class=\"campo_inserimento\">&nbsp; <select ".$disa." id=\"".$p['nome']."\" name=\"".$p['nome']."\" ".$fin." >\n";
          print $opt;
          if(isset($p['voci']))
          {
            $sel='';
          ...
          if($p['tipo']=='checkbox')
          {
            $ch="value=\"1\"";
          	if((isset($_SESSION['require']) && $_SESSION['require'][$p['nome']]=='1') || $p['nome']=='privacy')
      		{
      			...
      			if($p['nome']=='privacy')
      			{
      				$privacy='<textarea rows="5" cols="50" name="privacy" readonly>';
...
qualora ciò sia necessario per i futuri contatti al fine di definire possibili collaborazioni con Avv. Banchelli";
          ...
          print "<div class=\"label_form\"><label for=\"".$p['nome']."\">".$p['label']." </label></div>\n";
          print $privacy;
          print "<div class=\"campo_inserimento\">&nbsp; <input type=\"".$p['tipo']."\" id=\"".$p['nome']."\" name=\"".$p['nome']."\" size=\"28\" ".$ch." ".$value." /></div>\n";
          print "</div>\n";
        }
      }
    if($tab==1)
      ...
      print "<div class=\"riga_form\">\n";
        print"<div class=\"label_form\"><input type=\"submit\" value=\"Inserisci\" name=\"inserisci\" ".$dis."/></div>\n";
        if($dis!='')
      {
    ...
    {  
       ...
       foreach($this->_properties as $p)
      {
			...
			elseif($p['tipo']=='textarea')
         {
           print "<div class=\"riga_form\">\n";
           print "<div class=\"label_form\"><label for=\"".$p['nome']."\">".$p['label']."</label></div>\n";
           print "<div class=\"campo_inserimento\"><textarea id=\"".$p['nome']."\" name=\"".$p['nome']."\" cols=\"28\" rows=\"5\">".stripslashes($uno[$p['nome']])."</textarea></div>\n";
           print "</div>\n";
         }
         elseif($p['tipo']=='password')
         {
          print "<div class=\"riga_form\">\n";
          print "<div class=\"label_form\"><label for=\"".$p['nome']."\">".$p['label']." </label></div>\n";
          print "<div class=\"campo_inserimento\">&nbsp; <input type=\"".$p['tipo']."\" id=\"".$p['nome']."\" name=\"".$p['nome']."\" size=\"28\" value=\"".stripslashes($uno[$p['nome']])."\"  /></div>\n";
          print "</div>\n";
          print "<div class=\"riga_form\">\n";
          print "<div class=\"label_form\"><label for=\"c".$p['nome']."\">Conferma ".$p['label']." </label></div>\n";
          print "<div class=\"campo_inserimento\">&nbsp; <input type=\"".$p['tipo']."\" id=\"c".$p['nome']."\" name=\"c".$p['nome']."\" size=\"28\" value=\"".stripslashes($uno[$p['nome']])."\" /></div>\n";
          print "</div>\n";
         }
         elseif($p['tipo']=='select')
         {
          ...
          elseif($p['tipo']=='file')
          {
            ...
            {
              $img="<img src=\"../images/thbn/".$uno[$p['nome']]."?num=".rand()."\"/><br />\n";
              print '<div class="immagine">';
              print $img."<br />\n";
              print "<input type=\"button\" name=\"cancella_foto\" value=\"Cancella\" onclick=\"cancellaFoto('".$p['nome']."','".$id."','".$_GET['cosa']."');\" />";
              print '</div>';
            }
          ...
          else
          {
            ...
            elseif($p['nome']!='googlemap')
              {
              $ch="value=\"".stripslashes($uno[$p['nome']])."\"";
            }
            else 
            {
            	$ch="value=\"".htmlentities(stripslashes($uno[$p['nome']]))."\"";
            }
            print "<div class=\"label_form\"><label for=\"".$p['nome']."\">".$p['label']."</label></div>\n";
            print "<div class=\"campo_inserimento\"><input type=\"".$p['tipo']."\" id=\"".$p['nome']."\" name=\"".$p['nome']."\" size=\"28\" ".$ch." /></div>\n";
            print "</div>\n";
     ...
     if($tab==1)
      ...
      print "<div class=\"riga_form\">\n";
      print "<div class=\"label_form\"><input type=\"submit\" value=\"Modifica\" name=\"modifica\" /></div>\n";
      print "</div>\n";
    }
    ...
    function genera_lista($db,$nome,$fun=false,$id=0,$idnome='',$cond=false)
    {
     ...
     // print $query;
      $elenco=mysql_query("select * from ".$db.$fine)or die(mysql_error());
      ...
      while($el=mysql_fetch_assoc($elenco))
      {
         ...
         print "<form action=\"../include/funzioni/modifica.php?cosa=".$db."&id=".$el['id_'.$db].$url."&amp;nome=".$_GET['nome']."\"  method=\"post\" enctype=\"multipart/form-data\" onsubmit=\"return confirm('Si vuole procedere con la cancellazione di questo oggetto?');\">\n";
         if($db!='foto_prodotti')
         {
          print "<div class=\"label_form\">".$no."</div>\n";
          print "<div class=\"campo_inserimento\"><input type=\"button\" name=\"modifica\" value=\"Modifica\" onclick=\"javascript:location.replace('./modifica.php?cosa=".$db."&id=".$el['id_'.$db].$url."')\"/>\n";
         }
         else
         {
         ...
         print "<input type=\"submit\" name=\"cancella\" value=\"Cancella\" /></div>\n";
         print "</form>";
      }
    }
    ...
    function genera_lista_prod($negozio)
    {
      ...
      while($el=mysql_fetch_assoc($elenco))
      {
         ...
         print "<input type=\"button\" name=\"modifica\" value=\"Modifica categoria\" onclick=\"javascript:location.replace('./mod_prodotti_car.php?mod=1&amp;id_negozi=".$negozio."&amp;cosa=prodotti&id=".$el['id_prodotti']."&amp;scat=1')\"/>\n";
         print "<input type=\"submit\" name=\"cancella\" value=\"Cancella\" /></div>\n";
         print "</form>";
      }
    }
