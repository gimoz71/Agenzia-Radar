...
function costruisciPathLan($cosa,$immo,$lan)
    ...
    function immobiliHome($tipo,$lan)
    {
    	...
    	elseif ($tipo='vacanze')
    	{
    		$immobili=mysql_query("select * from immobili i, localita l, tipi t where t.id_tipi=i.id_tipi and l.id_localita=i.id_localita and (offerta=1 or residence=1)and pubblicato=1 order by home desc,rand() limit 0,6")or die(mysql_error());
    		$cosa='casa_vacanza';
    	}
    	$classeDe='';
    	if($lan=='de')
...
function elencoNews($tipo,$lan,$getor=false)
    ...
    {   
    	...
    	?>
                ...
                <a href="https://www.bookingeasy.it/pubblica/booking/booking2/frmBookingR.aspx?CR=110519F4FF1&CdR=<?php echo $im['id_immobili'];?>&PrimoGiorno=si" title="<?php echo PRENOTA;?>" class="form_esterno"></a>
                </div>
			    	<?php 
			    }
			    elseif($tipo=='residence' && $im['id_residence']==0)
			    {?>
			    <div class="bottone_prenota_<?php echo $_SESSION['lan']?>">
                <a href="https://www.bookingeasy.it/pubblica/booking/booking2/frmBooking2.aspx?CR=110519F4FF1&CdG=<?php echo $im['id_immobili'];?>" title="<?php echo PRENOTA;?>" class="form_esterno"></a>
                </div>
			    	<?php 
			    }
			    ?>
