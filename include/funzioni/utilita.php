...
{
	$messaggio=$msg."\n E' stata inviata la seguente richiesta da parte di un utente: \n\r";
...
function ins_foto($foto,$tipo,$id_im)
{
    ...
    $wtmkP=new Image(WATERMARKP);
    $wtmkB=new Image(WATERMARKB);
    $wtmkM=new Image(WATERMARKM);
		ini_set('memory_limit', '30M');
		$result = $imgt->CreateSourceImage();
		$waterB=$wtmkB->CreateSourceImage();
		$waterP=$wtmkP->CreateSourceImage();
		$waterM=$wtmkM->CreateSourceImage();
		if($result && strpos($upt->filename,'foto_g_immobile')===false)
	   {
	    	$imgt->SaveProportionateImageP(THUMB_DIR . '/' . $upt->filename, IMAGE_QUALITY, IMAGE_THBN_HEIGHT,IMAGE_THBN_WIDTH,$wtmkP);
			$imgt->SaveProportionateImageP(BIG_DIR . '/' . $upt->filename, IMAGE_QUALITY, IMAGE_BIG_HEIGHT,IMAGE_BIG_WIDTH,$wtmkB);
		    $upt->DeleteFile();
		    return $upt->filename;
		}
		elseif($result && strpos($upt->filename,'foto_g_immobile')!==false)
		{
			$imgt->SaveProportionateImageP(THUMB_DIR . '/' . $upt->filename, IMAGE_QUALITY, IMAGE_THBN_HEIGHT,IMAGE_THBN_WIDTH,$wtmkP);
			$imgt->SaveProportionateImageP(BIG_DIR . '/' . $upt->filename, IMAGE_QUALITY, IMAGE_BIG_HEIGHT,IMAGE_BIG_WIDTH,$wtmkB);
			$imgt->SaveProportionateImageP(MEDIE_DIR . '/' . $upt->filename, IMAGE_QUALITY, IMAGE_MEDIE_HEIGHT,IMAGE_MEDIE_WIDTH,$wtmkM);
		    $upt->DeleteFile();
		    return $upt->filename;
		}
		else
...
function ins_foto_old($foto,$tipo,$id_im)
{
	$tipot=explode('/',$foto['type']);
	if(	$tipot[0] != 'image'  )
	{
    ...
    $imgt = new Image(TMP_DIR . '/' . $upt->filename);
		...
		if($result && strpos($upt->filename,'foto_g_immobile')===false)
	   {
		...
		elseif($result && strpos($upt->filename,'foto_g_immobile')!==false)
		{
			$imgt->SaveProportionateImageP(THUMB_DIR . '/' . $upt->filename, IMAGE_QUALITY, IMAGE_THBN_HEIGHT,IMAGE_THBN_WIDTH);
			$imgt->SaveProportionateImageP(BIG_DIR . '/' . $upt->filename, IMAGE_QUALITY, IMAGE_BIG_HEIGHT,IMAGE_BIG_WIDTH);
			$imgt->SaveProportionateImageP(MEDIE_DIR . '/' . $upt->filename, IMAGE_QUALITY, IMAGE_MEDIE_HEIGHT,IMAGE_MEDIE_WIDTH);
		    $upt->DeleteFile();
		    return $upt->filename;
		}
		else
